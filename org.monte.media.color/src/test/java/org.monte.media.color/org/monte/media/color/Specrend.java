 ...
 *
 * This program is a Java conversion of speccrend.c
 *
 *
...
public class Specrend {
    ...
    public static class ColourSystem {
        ...
        public ColourSystem(String name, double xRed, double yRed, double xGreen, double yGreen, double xBlue, double yBlue, double[] white, double gamma) {
            this.name = name;
            this.xRed = xRed;
            this.yRed = yRed;
            this.xGreen = xGreen;
    ...
    public final static ColourSystem /* Name                  xRed    yRed    xGreen  yGreen  xBlue  yBlue    White point        Gamma   */ NTSCsystem = new ColourSystem("NTSC", 0.67, 0.33, 0.21, 0.71, 0.14, 0.08, IlluminantC, GAMMA_REC709),
            ...
            CIEsystem = new ColourSystem("CIE", 0.7355, 0.2645, 0.2658, 0.7243, 0.1669, 0.0085, IlluminantE, GAMMA_REC709),
            Rec709system = new ColourSystem("CIE REC 709", 0.64, 0.33, 0.30, 0.60, 0.15, 0.06, IlluminantD65, GAMMA_REC709);

    /*  	    	    	    UPVP_TO_XY
    Given 1976 coordinates u', v', determine 1931 chromaticities x, y
     */
    public void upvp_to_xy(double up, double vp, double[] xyc) {
        xyc[0] = (9 * up) / ((6 * up) - (16 * vp) + 12);
        xyc[1] = (4 * vp) / ((6 * up) - (16 * vp) + 12);
    }

    /*  	    	    	    XY_TO_UPVP
    Given 1931 chromaticities x, y, determine 1976 coordinates u', v'
     */
    public void xy_to_upvp(double xc, double yc, double[] upvp) {
        upvp[0] = (4 * xc) / ((-2 * xc) + (12 * yc) + 3);
        upvp[1] = (9 * yc) / ((-2 * xc) + (12 * yc) + 3);
    }
    ...
    public static void xyz_to_rgb(ColourSystem cs,
        ...
        rgb[0] = (rx * xyzc[0]) + (ry * xyzc[1]) + (rz * xyzc[2]);
        rgb[1] = (gx * xyzc[0]) + (gy * xyzc[1]) + (gz * xyzc[2]);
        rgb[2] = (bx * xyzc[0]) + (by * xyzc[1]) + (bz * xyzc[2]);
    }
    ...
    http://www.poynton.com/GammaFAQ.html
     */
    public static void gamma_correct(ColourSystem cs, double[] c) {
        double gamma;
        ...
        if (gamma == GAMMA_REC709) {
            ...
            if (c[0] < cc) {
                c[0] *= ((1.099 * pow(cc, 0.45)) - 0.099) / cc;
            } else {
                c[0] = (1.099 * pow(c[0], 0.45)) - 0.099;
            }
        } else {
    ...
    public static void gamma_correct_rgb(ColourSystem cs, double[] r, double[] g, double[] b) {
        gamma_correct(cs, r);
        gamma_correct(cs, g);
        gamma_correct(cs, b);
    }
    ...
    function SpecIntens, which is called with a series of
    ...
    public static void spectrum_to_xyz(SpecIntens spec_intens,
                                       double[] xyz) {
        int i;
        ...
        for (i = 0, lambda = 380; lambda < 780.1; i++, lambda += 5) {
            ...
            Me = spec_intens.specIntens(lambda);
     ...
     */
    public static double bb_spectrum(double wavelength, double bbTemp) {
    ...
    public static void bb_to_xyz(final double bbTemp,
        ...
        spectrum_to_xyz(new SpecIntens() {

            @Override
            public double specIntens(double wavelength) {
                return bb_spectrum(wavelength, bbTemp);
            }
        }, xyz);
    }
    ...
    public static void main(String[] args) {
        ...
        for (t = 1000; t <= 10000; t += 500) {
            ...
            System.out.printf(
                    "  %5.0f K      %.4f %.4f %.4f   ", t, xyz[0], xyz[1], xyz[2]);

            if (constrain_rgb(rgb)) {
                norm_rgb(rgb);
                System.out.printf(
                        "%.3f %.3f %.3f (Approximation)\n", rgb[0], rgb[1], rgb[2]);
            } else {
                norm_rgb(rgb);
                System.out.printf(
                        "%.3f %.3f %.3f\n", rgb[0], rgb[1], rgb[2]);
