...
/// Applies an image op to a buffered image.
///
/// @author Werner Randelshofer
public class ImageOpCodec extends org.monte.media.av.AbstractCodec {
    ...
    public final static FormatKey<BufferedImageOp> ImageOpKey = new FormatKey<>("imageOp", BufferedImageOp.class);
    ...
    public Format setOutputFormat(Format f) {
        if (!f.containsKey(ImageOpKey)) {
            throw new IllegalArgumentException("Output format must specify image op.");
        }
        ...
        Format fNew = super.setOutputFormat(f);
        return fNew;
    }
    ...
    private void applyImageOp(BufferedImage src, BufferedImage dst) {
        var imageOp = outputFormat.get(ImageOpKey);
        if (imageOp != null) {
            BufferedImage tmp = imageOp.filter(src, null);
