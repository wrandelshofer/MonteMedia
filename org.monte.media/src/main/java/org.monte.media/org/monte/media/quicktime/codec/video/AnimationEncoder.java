...
import org.monte.media.av.codec.video.VideoEncoderCore;
 ...
 */
public class AnimationEncoder extends org.monte.media.av.AbstractCodec {
    ...
    public int encode(Buffer in, Buffer out) {
        ...
        try {
            switch (vf.get(DepthKey)) {
                ...
                case 16: {
                    short[] pixels = ibta.getRGB15(in, r.width, r.height);
                    if (pixels == null) {
                        return CODEC_FAILED;
                    }
                ...
                case 24: {
                    WritableRaster raster = ibta.getPackedRgb24Raster(in, r.width, r.height);
                    if (raster == null) {
                        out.setFlag(DISCARD);
                        return CODEC_FAILED;
                    }
                ...
                case 32: {
                    WritableRaster raster = ibta.getPackedArgb32Raster(in, r.width, r.height);
                    if (raster == null) {
                        out.setFlag(DISCARD);
                        return CODEC_FAILED;
                    }
    ...
    public void encodeKey24(ImageOutputStream out, int[] data, int width, int height, int offset, int scanlineStride)
        ...
        out.setByteOrder(ByteOrder.BIG_ENDIAN);
        long headerPos = out.getStreamPosition();
        VideoEncoderCore avec = new VideoEncoderCore();
        ...
        for (int y = offset; y < ymax; y += scanlineStride) {
            ...
            for (; xy < xymax; ++xy) {
                ...
                if (repeatCount < 2) {
                    ...
                    if (literalCount > 126) {
                        out.write(literalCount); // Literal OP-code
                        avec.writeInts24(out, data, xy - literalCount + 1, literalCount);
                        literalCount = 0;
                    }
                } else {
                    if (literalCount > 0) {
                        out.write(literalCount); // Literal OP-code
                        avec.writeInts24(out, data, xy - literalCount, literalCount);
                        literalCount = 0;
                    }
                    out.write(-repeatCount); // Repeat OP-code
                    avec.writeInt24(out, v);
                    xy += repeatCount - 1;
                }
            }
