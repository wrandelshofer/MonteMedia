...
///   - `0x00 n data{n}0x00?`
///
/// Marks a literal run. `n`
///     gives the number of data bytes that follow. `n` must be between 3 and
///     255. If n is odd, a pad byte with the value 0x00 must be added.
///
///   - `n data`
///
/// Marks a repetition. `n`
///     gives the number of times the data byte is repeated. `n` must be
///     between 1 and 255.
///
///
/// Example:
...
public class RunLengthDecoder extends org.monte.media.av.AbstractCodec {
        ...
        switch (outputDepth) {
            case 8: {
                ...
                if (img == null || img.getWidth() != width || img.getHeight() != height || img.getType() != imgType) {
                    ColorModel cm = new ImageBufferToArray().getColorModel(in);
                    if (cm == null) {
    ...
    public boolean decode8(byte[] in, int off, int length, byte[] out, byte[] prev, int width, int height,
