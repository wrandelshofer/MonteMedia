...
public class TestImageFactory {
    ...
    ///
    /// @return test image
    public static BufferedImage createRec709Faces() {
        int size = 256;
        DirectColorModel cm = new DirectColorModel(Rec709ColorSpace.getInstance(), 24, 0xff0000, 0xff00, 0xff, 0x0, false, DataBuffer.TYPE_INT);
        var img = new BufferedImage(cm, cm.createCompatibleWritableRaster(size * 3, size * 2), false, null);
        var r = img.getRaster();
        for (int y = 0; y < 256; y++) {
            for (int x = 0; x < 256; x++) {
                img.setRGB(x, y, (x << 16) | (y << 8) | 255);
