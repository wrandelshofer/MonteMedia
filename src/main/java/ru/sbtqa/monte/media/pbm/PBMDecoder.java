/* @(#)PBMDecoder.java
 * Copyright © 2005-2010 Werner Randelshofer, Switzerland.
...
import static ru.sbtqa.monte.media.iff.IFFParser.ID_FORM;
import static ru.sbtqa.monte.media.iff.IFFParser.stringToID;
...
public class PBMDecoder implements IFFVisitor {
     ...
     * Chunk ID's.
     */
    protected final static int PBM_ID = stringToID("PBM ");
    protected final static int BMHD_ID = stringToID("BMHD");
    protected final static int CMAP_ID = stringToID("CMAP");
    protected final static int CRNG_ID = stringToID("CRNG");
    protected final static int DRNG_ID = stringToID("DRNG");
    protected final static int BODY_ID = stringToID("BODY");
    private final static int AUTH_ID = stringToID("AUTH");
    private final static int ANNO_ID = stringToID("ANNO");
    private final static int COPYRIGHT_ID = stringToID("(c) ");
    /**
    ...
    public void registerChunks(IFFParser iff) {
        iff.declareGroupChunk(PBM_ID, ID_FORM);
    ...
    public void visitChunk(IFFChunk group, IFFChunk chunk)
        ...
        for (IFFChunk chunk1 : chunks) {
            if (comment.length() > 0) {
                comment.append('\n');
            }
            comment.append(new String(chunk1.getData()));
        }
...
//memoryImageSource.putProperties(props);
    ...
    protected void decodeCMAP(IFFChunk chunk)
        ...
        if ((bmhdMasking & MSK_HAS_MASK) != 0) {
            arraycopy(red, 0, red, red.length / 2, red.length / 2);
            arraycopy(green, 0, green, green.length / 2, green.length / 2);
            arraycopy(blue, 0, blue, blue.length / 2, blue.length / 2);
...
cc = new DRNGColorCycle(rate, 273, min, max, //
    ...
    protected void decodeBODY(IFFChunk chunk)
        ...
        switch (bmhdCompression) {
            case CMP_NONE:
                arraycopy(data, 0, pixels, 0, data.length);
                break;
            case CMP_BYTE_RUN_1:
