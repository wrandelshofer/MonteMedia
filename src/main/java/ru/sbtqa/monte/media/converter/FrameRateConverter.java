 ...
 * 
 * Makes frames longer if the output time is behind the input time. Drops frames
 * if the output time runs away from the input time.
 * 
 * The output of the converter has a variable frame rate but it may still
 * contain identical frames. Thus an additional conversion step with
 * {@link FFRtoVFRConverter} is needed.
 * 
