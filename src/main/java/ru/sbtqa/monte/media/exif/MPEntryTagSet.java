...
/**
 * Syntethic tags for the entry information Multi-Picture format (MPF) tags as
 * found in MPO image files generated by Fujifilm Finepix Real 3D W1 cameras.
 * 
 * Source:
 * 
...
public class MPEntryTagSet extends TagSet {
    ...
    //
    /**
     * Individual Image Data Offset This field specifies the data offset to the
     * beginning (i.e. SOI marker) of an Individual Image. The field is
     * specified as a LONG value, and the byte order depends on MP Endian. This
     * offset is specified relative to the address of the MP Endian field in the
     * MP Header (see §5.2), unless the image is a First Individual Image, in
     * which case the value of the offset shall be NULL(00000000.H).
     */
    ...
    /**
     * Individual Image Size This field specifies the size of the image data
     * between the SOI and EOI markers of an Individual Image. The field is
     * specified as a LONG value, and the byte order depends on MP Endian.
     */
    ...
    public static MPEntryTagSet getInstance() {
        if (instance == null) {
            TIFFTag[] tags = {//
                ...
                /**
                 * Dependent Parent Image Flag: this flag shall be set to 1 if
                 * the Individual Image is the parent image of another dependent
                ...
                /**
                 * Dependent Child Image Flag: this flag shall be set to 1 if
                 * the Individual Image is the dependent child image of another
                 * Individual Image. Otherwise it shall be set to 0.*
                 */
                ...
                /**
                 * Representative Image Flag: if the Individual Image is a
                 * Representative Image, this flag is set to 1. Otherwise it is
                 * set to 0. There can be only one Individual Image in an MP
                ...
                /**
                 * Image Data Format: this code specifies the image data format
                 * of an Individual Image: 0 : JPEG, other : (Reserved).
                 */
                new TIFFTag("ImageDataFormat", TAG_ImageDataFormat, BYTE_MASK, new EnumValueFormatter(
                "JPEG", 0//
                )), //
                /**
                 * MP Type Code: this code specifies the MP Type of an
                 * Individual Image.
                 */
                ...
                /**
                 * Dependent Image x Entry Number(s) An Entry Number specifies
                 * the position of an arbitrary MP Entry relative to the first
                 * MP Entry. The field is specified as a SHORT value, and the
                 * byte order depends on MP Endian. - The Entry Number of the
                 * first MP Entry is 1. - If there are no Dependent Images then
                 * both the Dependent Image 1 Entry Number and the Dependent
                 * Image 2 Entry Number fields are set to 0. - If the Individual
                 * Image is a parent image (i.e. the Dependent Parent Image Flag
                 * in the Individual Image Attribute field is set to 1) the
                 * flags in this field are set depending on the number of
                 * Dependent Images: a.) If there is one dependent child image
                 * the Dependent Image 1 Entry Number is set to the entry number
                 * of the dependent child image, and the Dependent Image 2 Entry
                 * Number flag is set to 0. b.) If there are two Dependent
                 * Images, the entry number of each respective child image is
                 * specified in the Dependent Image 1 and Dependent Image 2
                 * Entry Number fields. - An Individual Image may have up to two
                 * Dependent Images.
                 */
