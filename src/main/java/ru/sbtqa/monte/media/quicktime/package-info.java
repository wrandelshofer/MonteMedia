 ...
 * 
 * A QuickTime movie has a time dimension defined by a time scale and a
 * duration. A movie always starts at time 0. The time scale defines the unit of
 * measure for the movie's time value. The duration specifies how long the movie
 * lasts.
 * 
 * 
 ...
 * 
 * 
 * A movie can contain one or more tracks. Each track refers to a media that can
 * be interpreted within the movie's time coordinate system. Each track begins
 * at the beginning of the movie. However, a track can end at any time. In
 * addition the media in the track may be offset from the beginning of the
 * movie. Tracks with media that does not commence at the beginning of a movie
 ...
 * has its own time coordinate system, which defines the media's time scale and
 * duration. A media's time coordinate system always starts at time 0, and it is
 * independend of the time coordinate system of the movie that uses its data.
 * 
 * 
 ...
 * the media. Rather, the media contains references to its data. The data may
 * reside in the movie file or in an external storage. The data referred to by
 * the media may be used by more than one movie, though the media itself is not
 * reused. If the data is stored in the movie, the samples of the media data can
 * be interleaved with each other. Interleaving can occur after each sample, or
 * after a chunk of samples.
 * 
 * 
 * Media 1 to 4 interleaved data:  1112221112221112221113332224
 * 
 * 
 * For more information about the QuickTime file format see the "QuickTime File
