...
import static ru.sbtqa.monte.media.FormatKeys.MediaType.META;
import static ru.sbtqa.monte.media.FormatKeys.MediaType.MIDI;
import static ru.sbtqa.monte.media.FormatKeys.MediaType.TEXT;
...
public class QuickTimeDeserializer {
    ...
    protected void parseRecursively(QTFFImageInputStream in, long remainingSize, QuickTimeMeta m) throws IOException {
        while (remainingSize > 0) {
            ...
            String t = atom.type;
            out.println("Collecting atom: " + atom.type + " size:" + atom.size + " headerSize=" + atom.headerSize);
                        ...
                        case "stsd":
                            switch (track.mediaType) {
                                ...
                                case VIDEO:
                                    ...
                                    break;
                                case MIDI:
                                    err.println("QuickTimeDeserializer.parseMIDISampleDescription not yet implemented.");
                                    break;
                                case TEXT:
                                    err.println("QuickTimeDeserializer.parseTextSampleDescription not yet implemented.");
                                    break;
                                case META:
                                    err.println("QuickTimeDeserializer.parseMetaSampleDescription not yet implemented.");
                                    break;
                                case FILE:
                                    err.println("QuickTimeDeserializer.parseFileSampleDescription not yet implemented.");
                                    break;
                                default:
    ...
    protected void parseMediaHeader(QTFFImageInputStream in, long remainingSize, QuickTimeMeta.Media m) throws IOException {
        ...
        if (languageCode < 0x800) {
            ...
            if (languageCode < LANGUAGE_CODES.length) {
                m.mediaLanguage = LANGUAGE_CODES[languageCode];
            } else {
    ...
    protected void parseSoundSampleDescription(QTFFImageInputStream in, long remainingSize, QuickTimeMeta.Media m) throws IOException {
        ...
        for (int i = 0; i < numberOfEntries; i++) {
            ...
            while (remainingEntrySize > 0) {
                ...
                out.println("stsd  atom:" + atomType);
                // FIXME Parse Atom
    ...
    protected void parseSyncSample(QTFFImageInputStream in, long remainingSize, QuickTimeMeta.Media m) throws IOException {
        ...
        if (numberOfEntries == 0) {
            ...
            m.syncSamples = new ArrayList<>(numberOfEntries);
            for (int i = 0; i < numberOfEntries; i++) {
