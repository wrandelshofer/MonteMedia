...
import static ru.sbtqa.monte.media.Registry.getInstance;
...
public class QuickTimeWriter extends QuickTimeOutputStream implements MovieWriter {
    ...
    public final static Format QUICKTIME = new Format(MediaTypeKey, FILE, MimeTypeKey, MIME_QUICKTIME);
    ...
    public int addTrack(Format fmt) throws IOException {
        if (fmt.get(MediaTypeKey) == VIDEO) {
            int t = addVideoTrack(fmt.get(EncodingKey),
                  ...
                  min(6000, fmt.get(FrameRateKey).getNumerator() * fmt.get(FrameRateKey).getDenominator()),
            ...
            return t;
        } else if (fmt.get(MediaTypeKey) == AUDIO) {
            // fill in unspecified values
            int sampleSizeInBits = fmt.get(SampleSizeInBitsKey, 16);
            ByteOrder bo = fmt.get(ByteOrderKey, BIG_ENDIAN);
            ...
            if (encoding == null || encoding.length() != 4) {
                if (signed) {
                    encoding = bo == BIG_ENDIAN ? "twos" : "sowt";
                } else {
                    encoding = "raw ";
                }
            }
    ...
    public int addAudioTrack(javax.sound.sampled.AudioFormat format) throws IOException {
        ...
        long timeScale = (int) floor(sampleRate);
        int sampleSizeInBits = format.getSampleSizeInBits();
        int numberOfChannels = format.getChannels();
        ByteOrder byteOrder = format.isBigEndian() ? BIG_ENDIAN : LITTLE_ENDIAN;
        int frameDuration = (int) (format.getSampleRate() / format.getFrameRate());
        int frameSize = format.getFrameSize();
        boolean isCompressed = format.getProperty("vbr") != null && ((Boolean) format.getProperty("vbr"));
        ...
        } else if (PCM_SIGNED.equals(enc)) {
            switch (sampleSizeInBits) {
                case 8:// Requires conversion to PCM_UNSIGNED!
                    qtAudioFormat = "raw ";
                    break;
                case 16:
                    qtAudioFormat = (byteOrder == BIG_ENDIAN) ? "twos" : "sowt";
                    break;
                case 24:
                    qtAudioFormat = "in24";
                    break;
        ...
        } else if (PCM_UNSIGNED.equals(enc)) {
            switch (sampleSizeInBits) {
                case 8:
                    qtAudioFormat = "raw ";
                    break;
    ...
    private Codec createCodec(Format fmt) {
        Codec[] codecs = getInstance().getEncoders(fmt.prepend(MimeTypeKey, MIME_QUICKTIME));
