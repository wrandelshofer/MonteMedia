...
public class IOStreams {
    ...
    /**
     * Copies up to the specified number of bytes from the remainder of the
     * source stream into the provided target stream.
     *
     * @param source the source stream
     * @param target the target stream
    ...
    public static long copy(InputStream source, ImageOutputStream target, long n) throws IOException {
        long count = 0L;
        byte[] b = new byte[8192];
        for (int read = source.read(b); read != -1 && count < n; read = source.read(b, 0, (int) min(b.length, n - count))) {
            target.write(b, 0, read);
            count += read;
        }
        return count;
