 ...
 * Copyright © 2001-2005 Werner Randelshofer, Switzerland.
...
import static ru.sbtqa.monte.media.image.Images.toBufferedImage;

/**
 * Draws a filled bevel border using an image and insets. The image must consist
 * of a bevel and a fill area.
 * 
 * The insets and the size of the image are used do determine which parts of the
 * image shall be used to draw the corners and edges of the bevel as well the
 * fill area.
 *
 * 
 * For example, if you provide an image of size 10,10 and a insets of size 2, 2,
 * 4, 4, then the corners of the border are made up of top left: 2,2, top right:
 * 2,4, bottom left: 2,4, bottom right: 4,4 rectangle of the image. The inner
 * area of the image is used to fill the inner area.
 *
 ...
 * <br>4.0 2005-04-25 Renamed from ImageBevelBorder to ImageBevelBorder. Because
 * we have now two optimized versions for Apple's Java 1.3 and one for Java 1.4
 * and above.
...
public class ImageBevelBorder implements Border {
    ...
    /**
     * This attribute is set to true, when the image is used to fill the content
     * area too.
     */
    private boolean fillContentArea;

    /**
     * Creates a new instance with the given image and insets. The image has the
     * same insets as the border.
     *
     * @param img TODO
    ...
    /**
     * Creates a new instance with the given image and insets. The image has
     * different insets than the border.
     *
     * @param img TODO
     * @param borderInsets TODO
    ...
    /**
     * Returns true if the border is opaque. This implementation always returns
     * false.
     *
     * @return TODO
     */
    ...
    /**
     * Paints the bevel image for the specified component with the specified
     * position and size.
     *
    ...
    public void paintBorder(Component c, Graphics gr, int x, int y, int width, int height) {
        ...
        if (width < left + right) {
            left = min(left, width / 2); //Math.max(0, left + (width - left - right) / 2);
            right = width - left;
        }
        if (height < top + bottom) {
            top = min(top, height / 2); //Math.max(0, top + (height - top - bottom) / 2);
            bottom = height - top;
        }
