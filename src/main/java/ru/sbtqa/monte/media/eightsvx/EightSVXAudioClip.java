 ...
 * since this is the only kind of audio data that JDK 1.1 supports (As far as I
 * know).
...
public class EightSVXAudioClip
    ...
    private long oneShotHiSamples_, // # samples in the high octave 1-shot part
          repeatHiSamples_, // # samples in the high octave repeat part
          samplesPerHiCycle_;  // # samples/cycle in high octave, else 0
    private int sampleRate_, // data sampling rate
 ...
 /* Accessors */
    ...
    /* Class methods */
    /**
     * This finds the volume correction needed when converting this stereo
     * sample to mono.
     *
     * @param stereo Stereo data linear 8. The first half of the array contains
     * the sound for the left speaker, the second half the sound for the right
     * speaker.
     * @return volumeCorrection Combining the two channels into one increases
     * the sound volume. This can exceed the maximum volume that can be
     * represented by the linear8 sample model. To avoid this, the volume must
     * be corrected to fit into the sample model.
     */
     ...
     * sound volume. This can exceed the maximum volume that can be represented
     * by the linear8 sample model. To avoid this, the volume must be corrected
     * to fit into the sample model.
     * @return TODO
     */
    ...
    /**
     * Resamples audio data to match the given sample rate and applies a lowpass
     * filter if necessary.
     *
     ...
     * @param destSampleRate Samples per second of the uLAW data (must be 8000
     * hertz).
     * @param volume The volume of the sound (1.0 == Unity). / public static
     * byte[] linear8ToULawWithLowpassAndHeader(byte[] linear8, int off, int
     * len, int sourceSampleRate, int destSampleRate, double volume) { //
     * Compute the factor between the source sample rate and destination sample
     * rate. double factor = (double)sourceSampleRate / (double)destSampleRate;
     *
     * int headersize = 24; int datasize = (sourceSampleRate == destSampleRate)
     * ? len : (int) Math.ceil(len / factor);
     *
     * // create the header byte[] header = { // Sun magic = ".snd" (byte)
     * 0x2e, (byte) 0x73, (byte) 0x6e, (byte) 0x64, // header size in bytes
     * (byte) (headersize >>> 24 & 0xff), (byte) (headersize >>> 16 & 0xff),
     * (byte) (headersize >>> 8 & 0xff), (byte) (headersize & 0xff), // data
     * size in bytes (byte) (datasize >>> 24 & 0xff), (byte) (datasize >>> 16 &
     * 0xff), (byte) (datasize >>> 8 & 0xff), (byte) (datasize & 0xff), /* //
     * data size in bytes (0) (byte) 0, (byte) 0, (byte) 0, 0, / // Sun uLaw
     * format (byte) 0, (byte) 0, (byte) 0, (byte) 1, // sample rate (only 8000
     * is supported by Java 1.1) (byte) (destSampleRate >>> 24 & 0xff), (byte)
     * (destSampleRate >>> 16 & 0xff), (byte) (destSampleRate >>> 8 & 0xff),
     * (byte) (destSampleRate & 0xff), // one channel for mono (don't care for
     * left or right speakers). (byte) 0, (byte) 0, (byte) 0, (byte) 1 };
     *
     * // Create the output array byte[] ulaw = new byte[datasize +
     * headersize]; System.arraycopy(header, 0, ulaw, 0, header.length);
     *
     * // uLaw encodes sound values from -8192 to +8191, // This is 32 times
     * more accurate than linear8. // We multiply this value into the volume
     * value // to save one computional step within the for-loop. volume =
     * volume * 32;
     *
     * if (sourceSampleRate == destSampleRate) { // No sample rate conversion
     * needed. // We simply convert each linear8 sample into a ulaw sample.
     *
     * int mask; for (int i=0; i < datasize; i++) { // pick a sample of the
     * linear 8-bit data, // convert it to two's complement, // and finally
     * multiply it with the volume factor. int ch = (int)( (0x80 - linear8[i +
     * off]) * volume);
     *
     * if (ch < 0) { ch = -ch; mask = 0x7f; } else { mask = 0xff; }
     *
     * if (ch < 32) { ch = 0xf0 | 15 - (ch / 2); } else if (ch < 96) { ch = 0xe0
     * | 15 - (ch - 32) / 4; } else if (ch < 224) { ch = 0xd0 | 15 - (ch - 96) /
     * 8; } else if (ch < 480) { ch = 0xc0 | 15 - (ch - 224) / 16; } else if (ch
     * < 992) { ch = 0xb0 | 15 - (ch - 480) / 32; } else if (ch < 2016) { ch =
     * 0xa0 | 15 - (ch - 992) / 64; } else if (ch < 4064) { ch = 0x90 | 15 - (ch
     * - 2016) / 128; } else if (ch < 8160) { ch = 0x80 | 15 - (ch - 4064) /  256; }
     * else                { ch = 0x80; }
     *
     * ulaw[i + headersize] = (byte)(mask & ch);
     * }
     * } else if (factor > 1.0) { // Sample rate conversion with downsampling
     * needed. // We have to apply a lowpass filter to remove sound //
     * frequencies that are higher than half of our destSampleRate // (this is
     * the Nyquist frequency).
     *
     * // Prepare the lowpass filter double resofreq = destSampleRate / 4; //
     * resonation frequency (must be < sampleRate / 4) double amp = 1.0; //
     * magnitude of the resonation frequency (?) double fx =
     * Math.cos(2*Math.PI*resofreq / sourceSampleRate); double c = 2-2*fx;
     * double r =
     * (Math.sqrt(2)*Math.sqrt(Math.pow(-(fx-1),3))+amp*(fx-1))/(amp*fx-1);
     * double pos = 0; double speed = 0;
     *
     *
     * int mask; // mask for the sign in the uLaw code for (int i=0; i < len;
     * i++) { speed = speed + (linear8[i+off] - pos) * c; pos += speed; speed *=
     * r;
     *
     * int j = (int)( i / factor ); if (ulaw[j + headersize] == 0) // try to
     * optimize a little bit { int ch = (int)( (0x80 - (byte)pos) * volume );
     *
     * if (ch < 0) { ch = -ch; mask = 0x7f; } else { mask = 0xff; }
     *
     * if (ch < 32) { ch = 0xf0 | 15 - (ch / 2); } else if (ch < 96) { ch = 0xe0
     ...
     * < 992) { ch = 0xb0 | 15 - (ch - 480) / 32; } else if (ch < 2016) { ch =
     * 0xa0 | 15 - (ch - 992) / 64; } else if (ch < 4064) { ch = 0x90 | 15 - (ch
     * - 2016) / 128; } else if (ch < 8160) { ch = 0x80 | 15 - (ch - 4064) /
     * 256; } else { ch = 0x80; }
     *
     * ulaw[j + headersize] = (byte)(mask & ch); } } } else { // Sample rate
     * conversion with upsampling needed. // We insert samples from our source
     * data multiple times into // the ulaw data array.
     *
     * int mask; for (int i=0; i < datasize; i++) { // pick a sample of the
     * linear 8-bit data, // convert it to two's complement, // and finally
     * multiply it with the volume factor. int ch = (int)( (0x80 - linear8[
     * (int)(off+i*factor) ]) * volume);
     *
     * if (ch < 0) { ch = -ch; mask = 0x7f; } else { mask = 0xff; }
     *
     * if (ch < 32) { ch = 0xf0 | 15 - (ch / 2); } else if (ch < 96) { ch = 0xe0
     ...
     * 256; } else { ch = 0x80; }
     *
     * ulaw[i + headersize] = (byte)(mask & ch); } } return ulaw;
     * }
     */
    /**
     * Applies a lowpass filter to the linear 8 data to avoid distortion when
     ...
     * hertz).
     * @param volume The volume of the sound (1.0 == Unity). / public static
     * byte[] linear8ToULawWithLowpass(byte[] linear8, int off, int len, int
     * sourceSampleRate, int destSampleRate, double volume) { // Compute the
     * factor between the source sample rate and destination sample rate. double
     * factor = (double)sourceSampleRate / (double)destSampleRate; int datasize
     * = (sourceSampleRate == destSampleRate) ? len : (int) Math.ceil(len /
     * factor);
     *
     * // Create the output array byte[] ulaw = new byte[datasize];
     *
     ...
     * r;
     *
     * int j = (int)( i / factor ); if (ulaw[j] == 0) // try to optimize a
     * little bit { int ch = (int)( (0x80 - (byte)pos) * volume );
     *
     * if (ch < 0) { ch = -ch; mask = 0x7f; } else { mask = 0xff; }
     *
     * if (ch < 32) { ch = 0xf0 | 15 - (ch / 2); } else if (ch < 96) { ch = 0xe0
     ...
     * 256; } else { ch = 0x80; }
     *
     * ulaw[j] = (byte)(mask & ch); } } } else { // Sample rate conversion with
     * upsampling needed. // We insert samples from our source data multiple
     * times into // the ulaw data array.
     *
     * int mask; for (int i=0; i < datasize; i++) { // pick a sample of the
     * linear 8-bit data, // convert it to two's complement, // and finally
     * multiply it with the volume factor. int ch = (int)( (0x80 - linear8[
     ...
     *
     * @return Linear 8 encoded audio data. / public static byte[]
     * lowpass(byte[] input, float c) { /* Algorithm taken from "Yehar's digital
     * sound processing tutorial for the braindead!" version 2001.05.02
     ...
     * /
     *
     * // Create the output array byte[] output = new byte[input.length];
     *
     * output[0] = 0; for (int t=1; t < input.length; t++) { output[t] = (byte)
     * (output[t - 1] + c * (input[t] - output[t - 1])); }
     *
     * return output;
     * }
     */
     ...
     *
     * @param input Linear8 encoded audio data.
     * @param resofreq Resonation frequency (must be < sampleRate / 4) @param r TODO
     * 0..1, but not 1
     *
     * @return Linear8 encoded audio data. / public static byte[]
     * lowpassV1(byte[] input, int sampleRate, float resofreq, float r) { /*
     * Algorithm taken from "Yehar's digital sound processing tutorial for the
     * braindead!" version 2001.05.02
     * http://www.student.oulu.fi/~oniemita/DSP/DSPSTUFF.TXT
     *
     * *** Fast lowpass with resonance v1 ***
     *
     * Parameters: resofreq = resonation frequency (must be < SR/4) r = 0..1,
     * but not 1
     *
     * Init: c = 2-2*cos(2*pi*resofreq / samplerate) pos = 0 speed = 0
     *
     * Loop: speed = speed + (input(t) - pos) * c pos = pos + speed speed =
     * speed * r output(t) = pos /
     *
     * // Create the output array byte[] output = new byte[input.length];
     *
     * // Init float c = (float) (2d - 2d * Math.cos(2d * Math.PI * resofreq /
     * sampleRate)); float pos = 0; float speed = 0;
     *
     * for (int t=0; t < input.length; t++) { speed = speed + (input[t] - pos) *
     * c; pos += speed; speed *= r;
     *
     * output[t] = (byte) pos; }
     *
     ...
     *
     * @param input Linear8 encoded audio data.
     * @param resofreq Resonation frequency (must be < sampleRate / 4) @param am TODO
     * p Magnitude at the resonation frequency
     *
     * @return Linear8 encoded audio data. / public static byte[]
     * lowpassV2(byte[] input, int sampleRate, float resofreq, float amp) { /*
     * Algorithm taken from "Yehar's digital sound processing tutorial for the
     * braindead!" version 2001.05.02
     * http://www.student.oulu.fi/~oniemita/DSP/DSPSTUFF.TXT
     *
     ...
     *
     * Parameters: resofreq = resonation frequency (must be < SR/4) amp =
     * magnitude at the resonation frequency
     *
     * Init: fx = cos(2*pi*resofreq / samplerate) c = 2-2*fx r =
     * (sqrt(2)*sqrt(-(fx-1)^3)+amp*(fx-1))/(amp*(fx-1)) pos = 0 speed = 0
     *
     * Loop: speed = speed + (input(t) - pos) * c pos = pos + speed speed =
     * speed * r output(t) = pos /
     *
     * // Create the output array byte[] output = new byte[input.length];
     *
     * // Init float fx = (float) Math.cos(2*Math.PI*resofreq / sampleRate);
     * float c = 2 - 2 * fx; float r = (float)
     * (Math.sqrt(2)*Math.sqrt(Math.pow(-(fx-1),3))+amp*(fx-1))/(amp*(fx-1));
     * float pos = 0; float speed = 0;
     *
     * for (int t=0; t < input.length; t++) { speed = speed + (input[t] - pos) *
     * c; pos += speed; speed *= r;
     *
     ...
     * p Magnitude at the resonation frequency
     *
     * @return Linear8 encoded audio data. / public static int[] lowpassV2(int[]
     * input, int sampleRate, float resofreq, float amp) { /* Algorithm taken
     * from "Yehar's digital sound processing tutorial for the braindead!"
     * version 2001.05.02 http://www.student.oulu.fi/~oniemita/DSP/DSPSTUFF.TXT
     *
     * *** Fast lowpass with resonance v2 ***
     *
     * Parameters: resofreq = resonation frequency (must be < SR/4) amp =
     ...
     * speed * r output(t) = pos /
     *
     * // Create the output array int[] output = new int[input.length];
     *
     * // Init double fx = Math.cos(2*Math.PI*resofreq / sampleRate); double c
     * = 2 - 2 * fx; double r =
     * (Math.sqrt(2)*Math.sqrt(Math.pow(-(fx-1),3))+amp*(fx-1))/(amp*(fx-1));
     * double pos = 0; double speed = 0;
     *
     ...
     */
    /**
     * Halfband lowpass. / public static byte[] halfbandLowpass(byte[] input) {
     * /* Algorithm taken from "Yehar's digital sound processing tutorial for
     * the braindead!" version 2001.05.02
     * http://www.student.oulu.fi/~oniemita/DSP/DSPSTUFF.TXT ** Halfband lowpass
     * ***
     *
     * b1 = 0.641339 b10 = -0.0227141 b2 = -3.02936 b3 = 1.65298 a0a12 =
     * 0.008097 b4 = -3.4186 a1a11 = 0.048141 b5 = 1.50021 a2a10 = 0.159244 b6 =
     * -1.73656 a3a9 = 0.365604 b7 = 0.554138 a4a8 = 0.636780 b8 = -0.371742
     * a5a7 = 0.876793 b9 = 0.0671787 a6 = 0.973529
     *
     * output(t) = a0a12*(input(t ) + input(t-12)) + a1a11*(input(t-1) +
     * input(t-11)) + a2a10*(input(t-2) + input(t-10)) + a3a9* (input(t-3) +
     * input(t-9 )) + a4a8* (input(t-4) + input(t-8 )) + a5a7* (input(t-5) +
     * input(t-7 )) + a6*input(t-6) + b1*output(t-1) + b2*output(t-2) +
     * b3*output(t-3) + b4*output(t-4) + b5*output(t-5) + b6*output(t-6) +
     * b7*output(t-7) + b8*output(t-8) + b9*output(t-9) + b10*output(t-10) /
     * double b1 = 0.641339, b10 = -0.0227141, b2 = -3.02936, b3 = 1.65298,
     * a0a12 = 0.008097, b4 = -3.4186 , a1a11 = 0.048141, b5 = 1.50021, a2a10 =
     * 0.159244, b6 = -1.73656, a3a9 = 0.365604, b7 = 0.554138, a4a8 = 0.636780,
     * b8 = -0.371742 , a5a7 = 0.876793, b9 = 0.0671787 , a6 = 0.973529;
     *
     * // Create the output array byte[] output = new byte[input.length];
     *
     * for (int t=12; t {@literal <} input.length; t++) { output[t] = (byte) (
     * a0a12*(input[t] + input[t-12]) + a1a11*(input[t-1] + input[t-11]) +
     * a2a10*(input[t-2] + input[t-10]) + a3a9* (input[t-3] + input[t-9 ]) +
     * a4a8* (input[t-4] + input[t-8 ]) + a5a7* (input[t-5] + input[t-7 ]) +
     * a6*input[t-6] + b1*output[t-1] + b2*output[t-2] + b3*output[t-3] +
     * b4*output[t-4] + b5*output[t-5] + b6*output[t-6] + b7*output[t-7] +
     * b8*output[t-8] + b9*output[t-9] + b10*output[t-10] ); } return output; }
     * /** Converts a buffer of signed 8bit samples to uLaw. The uLaw bytes
     * overwrite the original 8 bit values. The first byte-offset of the uLaw
     * bytes is byteOffset. It will be written sampleCount bytes. @param linear8
     * @param linear8 TODO
     * @return TODO
     */
     ...
     * Converts a buffer of signed 8bit samples to uLaw. The uLaw bytes
     * overwrite the original 8 bit values. The first byte-offset of the uLaw
     * bytes is byteOffset. It will be written sampleCount bytes.
     *
     * @param linear16 TODO
     * @return TODO
     */
     ...
     * make approximations, the compressor should adjust all the values
     * (forwards and backwards in time) for minimal overall distortion.
     */
    /**
     ...
     * 2*(n-2) byte dest buffer. Source buffer has a pad byte, an 8-bit initial
     * value, followed by n bytes comprising 2*(n) 4-bit encoded samples.
     *
     * @param source TODO
     * @return TODO
     */
